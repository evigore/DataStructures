...
private:
	...
	void _removeCircle(Leader* leader)
	{
		while (leader) {
			Trailer *trailer = leader->outs;
			Trailer *prev = trailer;
			while (trailer) {
				Trailer *next = trailer->next;

				if (leader->key == trailer->leader->key) {
					leader->inCount--;

					if (leader->outs == trailer)
						leader->outs = trailer->next;
...
int main(void)
{
	...
	cout << '\n' << (graph.hasCircle() ? "has loop" : "hasn't loop") << endl;
